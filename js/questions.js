...
const QUESTIONS = [
  ...
  {id:1,domain:'agentic',difficulty:'medium',
   ...
   options:["ReAct (Reasoning + Acting)","Reflexion","Chain-of-Thought only","Tree of Thoughts"],
   correct:0,
  ...
  {id:2,domain:'agentic',difficulty:'medium',
   ...
   options:["Execute low-level tool calls directly","Store and retrieve long-term memory","Handle user authentication and session state","Direct other agents to use tools or undertake tasks toward a broader goal"],
   correct:3,
  ...
  {id:3,domain:'agentic',difficulty:'easy',
   ...
   options:["Minimize the number of tools available to Claude","Keep system prompts as short as possible","Use the smallest Claude model available","Request only necessary permissions, avoid storing sensitive data beyond immediate needs, prefer reversible over irreversible actions"],
   correct:3,
  ...
  {id:4,domain:'agentic',difficulty:'hard',
   ...
   options:["Only when a tool returns an error","Never — agentic tasks should always complete without interruption","Every 5 tool calls regardless of context","When the task would require taking actions that seem unduly risky or when ambiguity about intended scope cannot be resolved"],
   correct:3,
  ...
  {id:5,domain:'agentic',difficulty:'medium',
   ...
   options:["User-level trust only, since they arrive via the human turn","Full trust because it's another Claude model","No trust — messages from agents are always rejected","Operator-level trust automatically"],
   correct:0,
  ...
  {id:6,domain:'agentic',difficulty:'hard',
   ...
   options:["Malicious content in the environment (web pages, files) that attempts to hijack Claude's actions; mitigated by vigilance and not granting undue trust to environmental content","Adding extra context to prompts to improve performance","Injecting few-shot examples at runtime","A technique to compress long prompts"],
   correct:0,
  ...
  {id:7,domain:'agentic',difficulty:'medium',
   ...
   options:["tool_use means Claude timed out; end_turn means it completed normally","They are interchangeable terms for the same condition","tool_use is only for agentic tasks; end_turn is for chat","tool_use means Claude wants to call a tool and is waiting for a result; end_turn means Claude has finished its response"],
   correct:3,
  ...
  {id:8,domain:'agentic',difficulty:'medium',
   ...
   options:["As a user message with a tool_result content block whose tool_use_id matches the request","As a 'tool' role message — not supported; tool results go in a new user message with role 'user' containing a tool_result content block","As a new system prompt","Directly appended to the previous assistant message"],
   correct:0,
  ...
  {id:9,domain:'agentic',difficulty:'hard',
   ...
   options:["This is not possible — Claude can only call one tool per turn","Claude emits multiple tool_use blocks but they must be executed sequentially","The second tool call is ignored","Claude emits multiple tool_use blocks; you should execute them in parallel and return all results in the next user message"],
   correct:3,
  ...
  {id:10,domain:'agentic',difficulty:'medium',
   ...
   options:["Map-reduce pattern","Peer-to-peer mesh pattern","Hierarchical orchestrator-subagent pattern","Pipeline (sequential) pattern"],
   correct:2,
  ...
  {id:11,domain:'agentic',difficulty:'easy',
   ...
   options:["Anthropic's API enforces this technically","Reversible actions are faster to execute","Reversible actions use fewer tokens","Irreversible actions (deleting files, sending emails, making purchases) cannot be undone if Claude makes an error, so caution preserves the ability to recover"],
   correct:3,
  ...
  {id:12,domain:'agentic',difficulty:'hard',
   ...
   options:["A synchronous chain","A streaming session","A batch process","A fully automated pipeline or 'agentic loop' — sometimes informally called an agent running in a loop"],
   correct:3,
  ...
  {id:13,domain:'agentic',difficulty:'medium',
   ...
   options:["A special 'agent_call' tool type built into the Anthropic API","Direct model-to-model WebSocket connections","Calling the Anthropic API from within a tool — the orchestrator's tool calls a function that itself makes an API request to Claude","Nested system prompts"],
   correct:2,
  ...
  {id:14,domain:'agentic',difficulty:'medium',
   ...
   options:["When you want maximum speed","When each step depends on the output of the previous step — e.g., research → draft → review → publish","When subtasks are independent and can run simultaneously","When all agents share the same system prompt"],
   correct:1,
  ...
  {id:15,domain:'agentic',difficulty:'hard',
   ...
   options:["Immediately terminate the entire agentic task","Include the error in the tool_result (with isError: true), allow Claude to reason about the failure, and decide whether to retry, try an alternative approach, or escalate to the user","Silently ignore the error and proceed","Retry the same call indefinitely until it succeeds"],
   correct:1,
  ...
  {id:16,domain:'agentic',difficulty:'medium',
   ...
   options:["A Claude instance with no tools","A fallback agent used when the primary agent fails","An agent that takes actions with real-world consequences (browsing the web, writing/executing code, managing files) as directed by an orchestrator","A smaller, cheaper version of Claude"],
   correct:2,
  ...
  {id:17,domain:'agentic',difficulty:'easy',
   ...
   options:["Ignore ambiguities — Claude will handle them automatically","Always ask the user to write a complete specification first","Start the task and resolve ambiguities as they arise","Identify and clarify ambiguities upfront before beginning, rather than interrupting mid-task or having to restart"],
   correct:3,
  ...
  {id:18,domain:'agentic',difficulty:'hard',
   ...
   options:["Performance degrades significantly in agent-to-agent communication","Claude models cannot communicate with each other","A compromised or malicious orchestrator could manipulate a subagent into performing unsafe actions; both models must independently apply safety behaviors","Claude models always trust each other by default"],
   correct:2,
  ...
  {id:19,domain:'agentic',difficulty:'medium',
   ...
   options:["force_tool: '<tool_name>'","system: 'always use tool X'","tool_choice: 'required'","tool_choice: { type: 'tool', name: '<tool_name>' }"],
   correct:3,
  ...
  {id:20,domain:'agentic',difficulty:'medium',
   ...
   options:["A Claude capability that allows it to interact with a computer's GUI — moving the cursor, clicking, typing, taking screenshots — to complete tasks","Claude's ability to write code that controls a computer","Claude's ability to manage cloud infrastructure","A special API for batch processing"],
   correct:0,
  ...
  {id:21,domain:'agentic',difficulty:'hard',
   ...
   options:["Claude cannot request permissions proactively","It violates the minimal footprint principle — unnecessary resource acquisition increases risk surface and undermines human oversight","It increases API costs","Tool rate limits prevent it technically"],
   correct:1,
  ...
  {id:22,domain:'agentic',difficulty:'medium',
   ...
   options:["Synchronous tools are only available for Claude Opus","Synchronous tool calls block until the result returns in the same conversation turn; async subagents run independently and may report back through a different mechanism (callback, polling, event)","Async subagents have more tokens available","There is no functional difference"],
   correct:1,
  ...
  {id:23,domain:'agentic',difficulty:'easy',
   ...
   options:["The task is run manually without any automation","A human must write all tool implementations","A human reviews every API response","A human is available to provide guidance, approval, or intervention at defined checkpoints during the agentic task"],
   correct:3,
  ...
  {id:25,domain:'agentic',difficulty:'medium',
   ...
   options:["Memory tools are required for all agentic tasks","Memory tools allow storing and retrieving information that persists across sessions or exceeds the context window size, enabling long-running agents to maintain state","They are equivalent — context window and memory tools serve the same purpose","Memory tools are faster than context window retrieval"],
   correct:1,
  ...
  {id:26,domain:'agentic',difficulty:'medium',
   ...
   options:["Calling the same tool multiple times in sequence","Running tools in parallel","Claude using the output of one tool call as the input to a subsequent tool call, building up complex results through a series of tool invocations","Caching tool results for reuse"],
   correct:2,
  ...
  {id:28,domain:'agentic',difficulty:'hard',
   ...
   options:["Complete the task anyway since the user requested it","Pause, explain the situation to the user, and ask for guidance rather than either proceeding with risky actions or abandoning without explanation","Automatically retry with reduced permissions","Abandon the task entirely without explanation"],
   correct:1,
  ...
  {id:29,domain:'agentic',difficulty:'medium',
   ...
   options:["Defined stopping points where the agent saves progress and optionally seeks human review before continuing","A token counting mechanism","A caching mechanism for API responses","A way to limit context window growth"],
   correct:0,
  ...
  {id:30,domain:'agentic',difficulty:'easy',
   ...
   options:["finish_reason","stop_reason","termination_type","end_condition"],
   correct:1,
  ...
  {id:31,domain:'agentic',difficulty:'hard',
   ...
   options:["Context window overflow in long agent loops","Claude convincing itself through a sequence of plausible-looking reasoning steps that an unsafe or harmful action is actually justified","Claude failing to call the right tool","Claude consuming too many tokens in a single response"],
   correct:1,
  ...
  {id:32,domain:'agentic',difficulty:'medium',
   ...
   options:["A testing pattern for agentic prompts","Isolating agent execution environments so that code execution, file access, or network calls cannot affect systems outside the intended scope","Limiting the number of tool calls per session","Storing agent state in a sandbox database"],
   correct:1,
  ...
  {id:33,domain:'agentic',difficulty:'easy',
   ...
   options:["Use extended thinking mode","Include the information in the system prompt","Train Claude on more recent data","Provide a tool that fetches the data when called, so Claude can request fresh information on demand"],
   correct:3,
  ...
  {id:34,domain:'agentic',difficulty:'hard',
   ...
   options:["JSON or XML in tool results and tool inputs, since these provide unambiguous structure that agents can reliably parse","Natural language only — structured data confuses agents","CSV format exclusively","Base64-encoded binary"],
   correct:0,
  ...
  {id:36,domain:'tools_mcp',difficulty:'easy',
   ...
   options:["Multi-Claude Protocol","Model Context Protocol","Machine Configuration Platform","Managed Compute Pipeline"],
   correct:1,
  ...
  {id:37,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["Host, Client, Server","Orchestrator, Tool, Memory","Model, Context, Protocol","Producer, Consumer, Broker"],
   correct:0,
  ...
  {id:38,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["gRPC and REST","HTTP and WebSocket","TCP and UDP","stdio (standard input/output) and HTTP with SSE (Server-Sent Events)"],
   correct:3,
  ...
  {id:39,domain:'tools_mcp',difficulty:'easy',
   ...
   options:["Tools, Resources, Prompts","Agents, Models, Embeddings","Functions, Variables, Classes","APIs, Webhooks, Streams"],
   correct:0,
  ...
  {id:40,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["id, function, parameters","tool_id, prompt, inputs","type, handler, schema","name, description, input_schema"],
   correct:3,
  ...
  {id:41,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["The description (tells Claude what the tool does, when to use it, and what it returns)","The required array in the input_schema","The tool's name (must be unique)","The input_schema (defines parameter types)"],
   correct:0,
  ...
  {id:42,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["text, image (base64), and resource (URI reference) content blocks","HTML and markdown only","Only plain text strings","JSON only"],
   correct:0,
  ...
  {id:43,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["Throw an exception in the API call","Return an empty string","Return an HTTP error status code","Set isError: true in the tool_result content block and include the error message in the content"],
   correct:3,
  ...
  {id:44,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["A load-balancing mechanism for MCP servers","Randomly selecting which tool to use","Taking sample data from a resource","An MCP feature allowing servers to request LLM completions from the host/client, enabling servers to use AI capabilities without direct API access"],
   correct:3,
  ...
  {id:45,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["URIs that clients expose to servers to indicate the boundaries of the client's accessible filesystem or data scope","Database root tables that MCP can access","Admin credentials for MCP servers","The top-level MCP server in a hierarchy"],
   correct:0,
  ...
  {id:47,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["type: 'string' with an 'enum' array listing the allowed values","type: 'string' with a 'values' array","type: 'choice'","type: 'variant'"],
   correct:0,
  ...
  {id:48,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["Store state in the tool's input parameters","Reconnect on every tool call for statelessness","Use the MCP resource primitive to store state","Maintain state within the MCP server process — each client gets a persistent connection with its own session state; initialize connections in the server's startup handlers"],
   correct:3,
  ...
  {id:49,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["Tools are synchronous; resources are async","Tools require authentication; resources do not","Tools cost tokens; resources are free","Tools are actions/functions that Claude invokes (with potential side effects); resources are data sources that provide read-only content addressable by URI"],
   correct:3,
  ...
  {id:50,domain:'tools_mcp',difficulty:'easy',
   ...
   options:["In the model configuration settings","In the 'tools' array in the API request body, each with name, description, and input_schema","As a separate API endpoint call","In the system prompt as a list"],
   correct:1,
  ...
  {id:51,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["Rate limiting tool calls","Using only read-only tools","Encrypting all tool results","Input sanitization and sandboxing — never directly interpolate user/model-supplied inputs into shell commands or code without validation, and execute in isolated environments to prevent injection attacks"],
   correct:3,
  ...
  {id:52,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["tool_choice: 'skip'","Remove the tools array entirely — or use tool_choice: { type: 'none' }","tool_choice: 'disabled'","tool_choice: { type: 'none' } — but note: the correct Anthropic API field is tool_choice: 'none' (a string in older SDKs) or { type: 'none' }"],
   correct:1,
  ...
  {id:54,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["Debug prompts for testing MCP servers","A way to override Claude's instructions","The system prompt sent to Claude","Reusable, parameterized prompt templates that MCP servers expose to clients — users or applications can invoke them to get a pre-structured prompt for common tasks"],
   correct:3,
  ...
  {id:55,domain:'tools_mcp',difficulty:'easy',
   ...
   options:["Parameters that require user confirmation","Parameters with security restrictions","Which parameters must be provided (vs. optional parameters with defaults)","Parameters that Claude must validate before calling the tool"],
   correct:2,
  ...
  {id:57,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["Error reporting to Anthropic","Sending alerts to users","Billing notifications for API usage","Servers notifying clients of changes (e.g., resource_list_changed, tools_list_changed) without waiting for a client request — enabling reactive updates when server capabilities change"],
   correct:3,
  ...
  {id:58,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["Any format — names are ignored by Claude","snake_case or kebab-case like get_weather_data or get-weather-data — lowercase with separators, descriptive and specific","CamelCase like getWeatherData","All uppercase like GET_WEATHER_DATA"],
   correct:1,
  ...
  {id:59,domain:'tools_mcp',difficulty:'easy',
   ...
   options:["Call the API again with the same messages array","Start a new conversation from scratch","Execute the tool(s), collect the result(s), and send a new user message containing tool_result content block(s) with the matching tool_use_id(s)","Send an empty user message to continue"],
   correct:2,
  ...
  {id:60,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["The user starts it manually","The MCP host application spawns the server as a child process and connects via the process's stdin/stdout","The MCP server starts itself","Claude Code manages all stdio processes automatically"],
   correct:1,
  ...
  {id:61,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["Return isError: true with a descriptive 'not found' error message so Claude knows the lookup failed vs. returning empty data","Return an empty string","Throw an unhandled exception","Return null without any error indication"],
   correct:0,
  ...
  {id:62,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["'title' is required; 'name' is optional","'title' controls how Claude describes the tool in its reasoning","'name' is the programmatic identifier used in tool calls (snake_case); 'title' is a human-readable display name shown in UIs. Claude uses 'name'; humans see 'title'","They serve the same purpose — use either one"],
   correct:2,
  ...
  {id:63,domain:'tools_mcp',difficulty:'hard',
   ...
   options:["Store them as environment variables that the MCP server reads at startup, never exposing them in tool definitions or results","Hard-code them in the tool implementation","Ask Claude to store them in its memory","Pass them as tool input parameters so Claude manages them"],
   correct:0,
  ...
  {id:64,domain:'tools_mcp',difficulty:'medium',
   ...
   options:["Authentication credentials","The list of available tools","Protocol version compatibility and capability negotiation between client and server — determining which features each side supports","The system prompt for Claude"],
   correct:2,
  ...
  {id:65,domain:'tools_mcp',difficulty:'easy',
   ...
   options:["64 characters","32 characters","256 characters","128 characters"],
   correct:0,
  ...
  {id:66,domain:'claude_code',difficulty:'easy',
   ...
   options:["Define tools available to Claude","Store conversation history","Provide persistent context and instructions to Claude Code — project conventions, architecture notes, important commands, and team guidelines that persist across sessions","Configure Claude's API parameters"],
   correct:2,
  ...
  {id:67,domain:'claude_code',difficulty:'medium',
   ...
   options:["Only user home directory","~/.claude/CLAUDE.md (global), the project root CLAUDE.md, and CLAUDE.md files in any parent directories up to the filesystem root, plus subdirectory CLAUDE.md files when working in that directory","Only the project root directory","Only directories explicitly specified in settings"],
   correct:1,
  ...
  {id:69,domain:'claude_code',difficulty:'hard',
   ...
   options:["Scheduled tasks run by Claude Code","Webhooks that notify external services when Claude responds","Plugins that extend Claude's capabilities","Shell commands that run at specific lifecycle events: PreToolUse (before a tool runs), PostToolUse (after a tool runs), Stop (when Claude finishes), and Notification (on alerts)"],
   correct:3,
  ...
  {id:70,domain:'claude_code',difficulty:'medium',
   ...
   options:["PostToolUse — it can log but not block","Notification — it intercepts tool calls","PreToolUse — it can block the tool call entirely (by exiting with code 2), modify behavior, or allow it to proceed","Stop — it runs before any tools"],
   correct:2,
  ...
  {id:71,domain:'claude_code',difficulty:'medium',
   ...
   options:["Only ANTHROPIC_API_KEY","Only variables defined in the hook script","CLAUDE_TOOL_NAME, CLAUDE_TOOL_INPUT (JSON), CLAUDE_TOOL_RESULT (JSON, for PostToolUse), and standard system environment variables","No environment variables — hooks use stdin/stdout"],
   correct:2,
  ...
  {id:72,domain:'claude_code',difficulty:'easy',
   ...
   options:["Store the current conversation","Clear conversation history","View token usage","View and edit Claude's persistent memory files (CLAUDE.md files) — adding, modifying, or removing persistent instructions and facts"],
   correct:3,
  ...
  {id:73,domain:'claude_code',difficulty:'medium',
   ...
   options:["Using the /mcp command only","In settings.json under the 'mcpServers' key — each entry specifies the server name, command, args, and optional env variables","MCP servers cannot be configured in Claude Code settings","By creating a .mcp file in the project root"],
   correct:1,
  ...
  {id:74,domain:'claude_code',difficulty:'medium',
   ...
   options:["Which files Claude can read","API rate limit overrides","Which tool invocations Claude Code is permitted to execute without prompting the user — specified as tool patterns in settings.json under 'permissions.allow'","Which Claude models can be used"],
   correct:2,
  ...
  {id:75,domain:'claude_code',difficulty:'easy',
   ...
   options:["Print all available tools","Run Claude Code in non-interactive mode, processing a prompt and printing the response to stdout — useful for scripting and CI pipelines","Print the current settings","Print the Claude Code version"],
   correct:1,
  ...
  {id:76,domain:'claude_code',difficulty:'medium',
   ...
   options:["Compress files in the project","Remove duplicate code","Minimize the Claude Code window","Compact the conversation history — summarizing earlier turns to reduce context size while preserving important information, allowing longer sessions without hitting context limits"],
   correct:3,
  ...
  {id:78,domain:'claude_code',difficulty:'medium',
   ...
   options:["Allows all tools without prompting","Restricts the session to only the specified tools — Claude can only use the tools in the provided comma-separated list","Lists all available tools","Adds new tools to the current session"],
   correct:1,
  ...
  {id:79,domain:'claude_code',difficulty:'easy',
   ...
   options:["/stats","/usage","/cost","/tokens"],
   correct:2,
  ...
  {id:80,domain:'claude_code',difficulty:'medium',
   ...
   options:["Using the /history command","Setting resume: true in settings.json","claude --resume (to resume the most recent session) or claude --resume <session-id> (to resume a specific session by ID)","--resume is not supported in Claude Code"],
   correct:2,
  ...
  {id:81,domain:'claude_code',difficulty:'hard',
   ...
   options:["'git' (tool name)","'Bash(git *)' — allows Bash tool calls where the command starts with 'git '","'Bash' (allows all bash)","'allow_git: true' in settings"],
   correct:1,
  ...
  {id:82,domain:'claude_code',difficulty:'medium',
   ...
   options:["Cancel the current session","Stop a running command","Run after Claude's turn ends — useful for triggering notifications, running formatters, updating logs, or performing cleanup after Claude finishes responding","Stop Claude from making any more tool calls"],
   correct:2,
  ...
  {id:83,domain:'claude_code',difficulty:'easy',
   ...
   options:["Remove all memory files","Clear the conversation history, starting a fresh context window while keeping settings and CLAUDE.md — useful when switching to a new task","Clear all tool permissions","Clear the terminal screen only"],
   correct:1,
  ...
  {id:84,domain:'claude_code',difficulty:'hard',
   ...
   options:["Hard-code in settings.json","Use the /apikey slash command","Store in ~/.claude/settings.json under 'apiKey'","Set ANTHROPIC_API_KEY as an environment variable — in ~/.bashrc or ~/.zshrc for interactive use, and as a CI/CD secret for automated pipelines. Never store in settings.json or CLAUDE.md."],
   correct:3,
  ...
  {id:85,domain:'claude_code',difficulty:'medium',
   ...
   options:["Controls font size: small, medium, large","Sets the log output format","Enables markdown or HTML rendering","Controls response format for --print mode: 'text' (plain text, default) or 'json' (structured JSON with role, content, session_id) — useful for scripting and parsing Claude's output"],
   correct:3,
  ...
  {id:86,domain:'claude_code',difficulty:'medium',
   ...
   options:["Set global: true in project settings.json","Add them to every project's CLAUDE.md","Add them to ~/.claude/CLAUDE.md — the global user-level memory file that Claude Code loads for every session regardless of project","Use the /global command"],
   correct:2,
  ...
  {id:87,domain:'claude_code',difficulty:'hard',
   ...
   options:["HTTP status codes from external services","Syntax errors in code","When Claude Code sends a notification event — typically when a long-running task completes, when Claude needs user attention, or at configurable trigger points in workflows","Tool call errors"],
   correct:2,
  ...
  {id:88,domain:'claude_code',difficulty:'easy',
   ...
   options:["Review conversation history","Trigger a code review of the current changes or specified code — Claude performs a review focusing on correctness, style, potential bugs, and improvements","Review installed MCP servers","Review API usage costs"],
   correct:1,
  ...
  {id:89,domain:'claude_code',difficulty:'medium',
   ...
   options:["The default Claude model to use for the session (e.g., 'claude-opus-4-7', 'claude-sonnet-4-6') — overridable per-session with the --model flag","The version of Claude Code to use","The language model temperature","The embedding model for semantic search"],
   correct:0,
  ...
  {id:90,domain:'claude_code',difficulty:'hard',
   ...
   options:["They are identical in functionality","PostToolUse is for read tools; PreToolUse is for write tools","PostToolUse can modify the tool result; PreToolUse cannot","PreToolUse validates/blocks tool calls before execution; PostToolUse processes results after execution — e.g., auto-formatting files after edits, logging results, triggering downstream actions based on what the tool returned"],
   correct:3,
  ...
  {id:91,domain:'claude_code',difficulty:'medium',
   ...
   options:["A JSON string of the tool's input parameters — e.g., for a Bash tool call, it would contain the command being executed","The previous tool's output","The session ID","The name of the tool being called"],
   correct:0,
  ...
  {id:93,domain:'claude_code',difficulty:'hard',
   ...
   options:["User-level settings completely replace project-level permissions","The 'allow' arrays are merged — both sets of rules apply — so you get the union of allowed tools from both settings files","A conflict error is thrown","Only user-level permissions apply when both are present"],
   correct:1,
  ...
  {id:94,domain:'claude_code',difficulty:'medium',
   ...
   options:["--batch","--print (or -p) combined with the prompt as an argument — this runs non-interactively, processing the prompt and exiting","--headless","--ci"],
   correct:1,
  ...
  {id:95,domain:'claude_code',difficulty:'medium',
   ...
   options:["They configure Claude Code's tool permissions","They inform Claude how to build, test, and validate the project — so Claude knows to run the correct commands rather than guessing, and can verify its changes work correctly","They replace the project's Makefile","They are automatically executed every time Claude Code starts"],
   correct:1,
  ...
  {id:96,domain:'claude_code',difficulty:'hard',
   ...
   options:["An npm script name","A docker-compose service name","An object with 'hooks' array where each entry has 'type' (PreToolUse/PostToolUse/Stop/Notification), optional 'tools' filter array, and 'command' (shell command to execute)","A Python function reference"],
   correct:2,
  ...
  {id:97,domain:'claude_code',difficulty:'easy',
   ...
   options:["Set up a new MCP server","Install Claude Code extensions","Generate an initial CLAUDE.md file for the current project — Claude analyzes the codebase and creates a starter memory file with architecture notes, conventions, and commands","Initialize a new git repository"],
   correct:2,
  ...
  {id:98,domain:'claude_code',difficulty:'medium',
   ...
   options:["Add the server to .gitignore","Define it in the project-level .claude/settings.json rather than the user-level ~/.claude/settings.json — project settings are local to that repository","Set 'scope: project' in the MCP server config","Use the --mcp flag with the server name"],
   correct:1,
  ...
  {id:99,domain:'claude_code',difficulty:'hard',
   ...
   options:["An extended thinking mode for code analysis","A built-in code review tool","A multi-agent cloud review skill triggered via /ultrareview that launches a comprehensive review of the current branch or a specific GitHub PR","A type of memory compression"],
   correct:2,
  ...
  {id:101,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["In the system prompt, separate from the document","After the document — instructions placed after the content Claude should read tend to be followed more accurately for long documents","It makes no difference where instructions appear","Before the document only"],
   correct:1,
  ...
  {id:102,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Using only 2-3 words in the system prompt","Limiting Claude to a few tokens per response","Providing Claude with examples of the desired input-output behavior in the prompt — most effective for tasks with specific formatting requirements, niche styles, or complex output structures that are hard to describe verbally","Using a very small model for simple tasks"],
   correct:2,
  ...
  {id:103,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Use JSON instead of XML for all structured data","Use XML tags to clearly demarcate distinct sections of the prompt (e.g., <instructions>, <document>, <examples>, <output_format>) — Claude is explicitly trained to recognize and use XML structure","Use only <br> and <b> HTML tags","XML tags are not recommended — use markdown instead"],
   correct:1,
  ...
  {id:104,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["Whether Claude uses tools","The randomness/creativity of responses — temperature 0 produces the most deterministic output; higher values (up to 1) introduce more variation and creativity","Claude's response speed","The maximum response length"],
   correct:1,
  ...
  {id:106,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Chaining multiple API calls together","Breaking long prompts into shorter chains","Prompting Claude to think through a problem step-by-step before giving a final answer — improves accuracy on complex reasoning tasks, math problems, and multi-step logical deductions","Linking tools together in sequence"],
   correct:2,
  ...
  {id:107,domain:'prompt_eng',difficulty:'hard',
   ...
   options:["CoT with zero examples","Using chain-of-thought in the system prompt only","Disabling chain-of-thought","Triggering chain-of-thought reasoning without providing examples, using instructions like 'Think step by step' or 'Let's reason through this' — Claude generates the reasoning process itself rather than following demonstrated examples"],
   correct:3,
  ...
  {id:109,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["Setting Claude's temperature based on its role","Defining which API role calls Claude's response","Assigning Claude a user role for access control","Giving Claude a persona or role ('You are an expert data scientist...') to activate relevant knowledge, style, and perspective — helps Claude produce more focused, domain-appropriate responses"],
   correct:3,
  ...
  {id:110,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Error codes that stop the API request","Patterns that indicate prompt injection","Sequences that indicate Claude has started a tool call","Custom string(s) that, when encountered in Claude's output, cause generation to stop immediately — useful for controlling response length, output format boundaries, or preventing over-generation"],
   correct:3,
  ...
  {id:112,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Never — positive instructions are always clearer","Use sparingly as a complement to positive instructions when there's a specific failure mode to prevent — but positive instructions (what TO do) should be primary since they're more concrete and actionable","Always — every prompt should have both positive and negative instructions","Only in system prompts, never in user turns"],
   correct:1,
  ...
  {id:115,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["It activates more deliberate, thorough processing — especially useful for complex tasks, reducing hasty errors and encouraging consideration of edge cases and multiple perspectives","No effect — Claude always thinks carefully","It increases temperature automatically","It forces Claude to use extended thinking mode"],
   correct:0,
  ...
  {id:116,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Use numbered lists only","Use consistent, clear structure — wrap examples in XML tags like <example><input>...</input><output>...</output></example> to clearly separate them from instructions and from each other","Use code blocks for all examples regardless of content","Embed them in the system prompt only"],
   correct:1,
  ...
  {id:117,domain:'prompt_eng',difficulty:'hard',
   ...
   options:["top-p is for input tokens; temperature is for output tokens","top-p controls creativity; temperature controls speed","top-p limits the token selection pool to the smallest set of tokens whose cumulative probability exceeds p (e.g., top-p 0.9 = use tokens covering 90% of probability mass); temperature scales probabilities before selection. They interact: low temperature makes the pool smaller even at high top-p","They are different names for the same parameter"],
   correct:2,
  ...
  {id:121,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["'human'","'user' — messages in the messages array alternate between 'user' (human inputs) and 'assistant' (Claude's responses)","'human' for API v1, 'user' for API v2","'input'"],
   correct:1,
  ...
  {id:122,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["Use numbered lists or clearly labeled sections with XML tags — structured, explicit formatting with one instruction per line improves instruction-following more reliably than dense prose","Repeat the instructions three times","Put all instructions in one long paragraph","Use all-caps for important instructions"],
   correct:0,
  ...
  {id:125,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["When the system prompt is too long and gets truncated","System prompt instructions conflicting with user instructions","System prompt content accidentally appearing in Claude's response to users — prevented by instructing Claude to keep the system prompt confidential and never repeat it verbatim","API keys being exposed in system prompts"],
   correct:2,
  ...
  {id:126,domain:'prompt_eng',difficulty:'hard',
   ...
   options:["The empirically observed tendency for models to pay less attention to content in the middle of a very long context window — content at the beginning and end is better utilized. Mitigation: put critical instructions at the start and/or end, not only in the middle","Truncation of context window","Forgetting the beginning of a conversation","Context that gets cut off due to token limits"],
   correct:0,
  ...
  {id:127,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Always use single prompts for efficiency","Multi-turn is only for chat applications","Multi-turn is better when the task benefits from iterative refinement, user feedback between steps, or when intermediate results should be validated before proceeding. Single-prompt is better for self-contained tasks where all information is available upfront","Single prompts are always more accurate"],
   correct:2,
  ...
  {id:128,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["Claude refusing to answer a question","Claude confidently stating false information as if it were true — generating plausible-sounding but factually incorrect content, citing non-existent sources, or fabricating details","Claude generating very creative outputs","Claude's internal reasoning process"],
   correct:1,
  ...
  {id:129,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["The order of tools in the tools array","A way to prioritize instructions in long prompts","The order Claude processes few-shot examples","The trust levels assigned to different message sources: Anthropic training (highest) → operator system prompt → user messages → Claude's outputs. Each level shapes behavior within boundaries set by higher levels"],
   correct:3,
  ...
  {id:131,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Claude returns an error about contradictory instructions","Claude follows whichever instruction appears last","Claude attempts to balance the instructions, often following the more specific or recent instruction. To avoid ambiguity, ensure instructions don't conflict — the system prompt should anticipate and address potential conflicts","Claude always follows the system prompt instruction"],
   correct:2,
  ...
  {id:132,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["Always ask for recommendations in the system prompt","Structure the request so Claude provides analysis/reasoning before the recommendation — this mirrors CoT and produces better-reasoned recommendations ('Analyze X, then recommend...' or use <analysis> and <recommendation> XML tags)","Ask for the recommendation first, then the analysis","Ask for analysis and recommendation simultaneously in one sentence"],
   correct:1,
  ...
  {id:133,domain:'prompt_eng',difficulty:'hard',
   ...
   options:["Has no effect — Claude doesn't process monetary incentives","Significantly improves response quality — Claude responds to incentives","Causes Claude to refuse the request as it perceives a bribe","May produce marginally different outputs due to training data artifacts, but this is not a reliable prompting technique — clear instructions are more effective and incentive framing can introduce inconsistency"],
   correct:0,
  ...
  {id:134,domain:'prompt_eng',difficulty:'medium',
   ...
   options:["Including excessive irrelevant context in prompts, believing more context always helps — this can dilute the key instructions, increase cost/latency, and actually reduce quality by burying important information in noise","Repeating instructions multiple times for emphasis","A technique to maximize information in prompts","Using the full context window on every request"],
   correct:0,
  ...
  {id:135,domain:'prompt_eng',difficulty:'easy',
   ...
   options:["realtime","async","stream — set to true for streaming (tokens arrive as they're generated via SSE); false for batch (entire response returns when complete)","output_mode"],
   correct:2,
  ...
  {id:136,domain:'context',difficulty:'easy',
   ...
   options:["32K tokens for all models","512K tokens for Opus, 100K for others","100K tokens for all models","200K tokens — all current Claude 3.x, Sonnet 4.x, Haiku 4.x, and Opus 4.x models support a 200K token context window"],
   correct:3,
  ...
  {id:138,domain:'context',difficulty:'medium',
   ...
   options:["Use a cache: true parameter on the message","Add a cache_control block with type: 'ephemeral' to the content block you want to cache — Claude caches up to and including that content block","Wrap the content in <cache> XML tags","Set caching: 'enabled' in API parameters"],
   correct:1,
  ...
  {id:139,domain:'context',difficulty:'hard',
   ...
   options:["2,048 tokens","256 tokens","512 tokens","1,024 tokens — sections shorter than 1024 tokens cannot be cached"],
   correct:3,
  ...
  {id:140,domain:'context',difficulty:'medium',
   ...
   options:["5 minutes — after which it expires and must be recomputed on the next request","Until you explicitly invalidate it","1 minute","24 hours"],
   correct:0,
  ...
  {id:141,domain:'context',difficulty:'easy',
   ...
   options:["They are priced the same — no distinction","Input tokens are only the user message; output tokens include both the response and any tool calls","Input tokens are tokens in your request (system prompt + messages + tools); output tokens are tokens Claude generates in its response. They are priced differently — output tokens typically cost more than input tokens","There is no distinction — the API only counts total tokens"],
   correct:2,
  ...
  {id:142,domain:'context',difficulty:'medium',
   ...
   options:["POST /v1/messages/count_tokens — send the same request body and receive a token count without generating a response, useful for estimating costs and managing context window usage","The usage field in the response — but only after generation","GET /v1/token-count","The tokenize endpoint"],
   correct:0,
  ...
  {id:143,domain:'context',difficulty:'hard',
   ...
   options:["Just increase max_tokens","Use a smaller model with a larger context window","Sliding window truncation (remove oldest messages to maintain recency) and summarization (compress older context into a summary). The choice depends on whether exact earlier content matters or just the gist","Truncation only (delete oldest messages) or restart the conversation"],
   correct:2,
  ...
  {id:145,domain:'context',difficulty:'easy',
   ...
   options:["This is actually backwards — instructions should come after documents because the 'lost in the middle' phenomenon means instructions at the end are better retained after reading a long document","Document placement doesn't affect output quality","Documents must always come first for the API to process correctly","For large documents, instructions after the document are more reliably followed — Claude processes the document first and then applies the instruction to it while it's fresh"],
   correct:3,
  ...
  {id:146,domain:'context',difficulty:'hard',
   ...
   options:["Tokens that were pre-loaded into the cache","Total tokens across all cached requests","How many input tokens were served from the prompt cache rather than being recomputed — these are billed at the reduced cache read rate (approximately 10% of standard input token price), indicating cache savings were achieved","The number of tokens available for future caching"],
   correct:2,
  ...
  {id:147,domain:'context',difficulty:'medium',
   ...
   options:["On every content block for maximum caching","Only on the first content block","On the user question — cache the whole thing","On the last content block of the stable content — e.g., at the end of the system prompt (if it's large and stable) or at the end of the document (if the document is fixed across calls but the question varies)"],
   correct:3,
  ...
  {id:148,domain:'context',difficulty:'easy',
   ...
   options:["1,000 words — tokens and words are roughly equivalent","2,000 words","750 words — a common rule of thumb is ~750 English words per 1,000 tokens (or ~1.3 tokens per word)","500 words"],
   correct:2,
  ...
  {id:150,domain:'context',difficulty:'medium',
   ...
   options:["The context window is the maximum tokens accepted; effective context is how well the model actually uses all that context — very long contexts may have reduced quality due to attention limitations. Practical performance on content deep in the middle of a 200K context may differ from a 10K context","Effective context refers to cached tokens only","Context window is measured in words; effective context in tokens","They are the same thing"],
   correct:0,
  ...
  {id:151,domain:'context',difficulty:'easy',
   ...
   options:["A prompt caching technique","A way to train Claude on new data","A technique for dynamically retrieving relevant documents from a knowledge base and inserting them into the context window for each query — managing context by providing only the most relevant information rather than the entire knowledge base","A method for compressing context"],
   correct:2,
  ...
  {id:153,domain:'context',difficulty:'medium',
   ...
   options:["Implement a summarization step: periodically summarize older turns into a concise summary, then replace those turns with the summary in the messages array — preserving semantics while reducing token count","Send only the last message each time","Stop the agent and start over","Switch to a model with a larger context window"],
   correct:0,
  ...
  {id:154,domain:'context',difficulty:'easy',
   ...
   options:["4 — you can mark up to 4 content blocks with cache_control in a single request","Unlimited","10","1"],
   correct:0,
  ...
  {id:156,domain:'context',difficulty:'medium',
   ...
   options:["Context window size doesn't affect cost","Cost is fixed per API call regardless of token count","Larger prompts (more input tokens) cost more. Since the Anthropic API charges per token, longer context = higher cost per call. Prompt caching reduces this for repeated stable content. Output tokens cost more per token than input tokens.","Context window size only affects latency, not cost"],
   correct:2,
  ...
  {id:159,domain:'context',difficulty:'easy',
   ...
   options:["Extended thinking tokens are free in all turns","Thinking blocks are only available in the first turn","When you include Claude's full response (including thinking blocks) in the conversation history, thinking tokens become part of the input for the next turn and are billed as input tokens. You can choose to strip thinking blocks from history to reduce costs.","They are automatically discarded — thinking never persists to the next turn"],
   correct:2,
  ...
  {id:160,domain:'context',difficulty:'hard',
   ...
   options:["Use RAG: chunk and index the manual, retrieve relevant sections per query, insert only the 3-5 most relevant chunks into context — far more token-efficient than the full manual for most queries","Include the full manual in every request","Cache the entire manual with prompt caching","Summarize the manual into 10 pages and include the summary"],
   correct:0,
